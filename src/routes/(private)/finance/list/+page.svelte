...
<MainPage
	title="Książka finansowa"
	subtitle="Lista operacji finansowych"
	gotoUrl="/finance/finance_details"
	buttonName="Dodaj transakcję"
>
	<TableView labelList={['Nazwa', 'Numer dokumentu', 'Data', 'Kategoria', 'Kwota']}>
		{#each data.operations as operation}
			...
			>
				...
				<td>{operation.invoice_number || 'Brak numeru'}</td>
				...
				/>
				<td class="text-sm">{operation.amount > 0 ? '+' : '-'} {Math.abs(operation.amount)} PLN</td>
			</tr>
		{/each}
	</TableView>
</MainPage>
