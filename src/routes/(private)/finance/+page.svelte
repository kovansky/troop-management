<script lang="ts">
	...
	//on change of innerWidth, update isMobile
	...
	function renderChart() {
		...
		// list of categories and their percents
...
<main class="h-screen pb-16 overflow-auto">
				...
				<div class="md:w-2/3 w-full">
					...
					<table class="w-full whitespace-no-wrap table-auto">
						...
						<tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
							...
							<!-- same row as many times as it fits on the screen (responsive) -->
							{#each data.last_operations as operation}
								<tr class="text-gray-700 dark:text-gray-400 hover:bg-gray-100 cursor-pointer">
									...
									<ColorTag
										...
										title={
										operation.finance_categories?.name || (operation.fk_fee == null ? 'Brak kategorii' : 'Składka')
										}
									/>
									<!-- <td class="px-4 py-3 text-sm">WOWxd</td> -->
									<td class="px-4 py-3 text-sm">{operation.amount > 0 ? '+' : '-'} {Math.abs(operation.amount)} PLN</td>
