...
export async function load({ locals: { supabase, getSession } }) {
  ...
  const { data: peopleUid, error: db_error } = await supabase.from("people").select(selectData).not("fk_user_id", "is", null);
  const { data: peopleRole, error: db_error1 } = await supabase.from("people").select(selectData).not("roles", "is", null).eq("roles.is_admin", true);
  let people = peopleUid.concat(peopleRole);
  people = people.filter((person, index, self) =>
