...
function validatePerson(body) {
	...
	if (body.parent_phone && !/^\d{9}$/.test(body.parent_phone))
		return json({ status: 400, body: 'Numer telefonu rodzica jest niepoprawny' });
	if (body.parent_email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.parent_email))
		return json({ status: 400, body: 'Email rodzica jest niepoprawny' });
	return null;
}
