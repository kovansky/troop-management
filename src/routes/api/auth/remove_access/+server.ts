...
export async function POST({ request, locals: { getSession, supabaseService, supabase, getServiceSession } }): Promise<{ status: number, body }> {
    ...
    if (!serviceSession?.user?.role) return json({ status: 401, body: 'Użytkownik niezalogowany - odśwież sesję' });
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return json({ status: 400, body: 'Email jest niepoprawny' });
    ...
    const { data: data2, error: error2 } = await supabase.from('people').update({ fk_user_id: data.user.id, admin_since: new Date().toUTCString()}).eq('id', user_id).select();
