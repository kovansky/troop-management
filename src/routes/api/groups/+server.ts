...
export async function POST({ request, locals: { getSession, supabase } }): Promise<{ status: number, body }> {
    ...
    const { data: small_groups, error: error2 } = await supabase.from('small_groups').upsert(data).select();
    if (error2) return json({ status: 500, body: error2 });

    const members = formData.getAll('people');
    for (let i = 0; i < members.length; i++) {
        const { error } = await supabase.from('group_person').insert([{ fk_small_group_id: small_groups[0].id, fk_person_id: parseInt(members[i].toString()) }]);
