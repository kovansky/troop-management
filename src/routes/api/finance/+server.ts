...
async function updateInsertFinanceHistory(body, { locals: { supabase, getSession } }) {
    ...
    if (!body.id) {
        ...
        const { data: finance, error: db_error } = await supabase.from('finance_history').insert(data).select();
        if (db_error) return json({ status: 500, body: db_error });
        console.log(db_error)
        return json({ status: 201, body: finance });
    } else {
        console.log('updating');
        const { data: finance, error: db_error } = await supabase.from('finance_history').upsert(data).select();
        if (db_error) return json({ status: 500, body: db_error });
